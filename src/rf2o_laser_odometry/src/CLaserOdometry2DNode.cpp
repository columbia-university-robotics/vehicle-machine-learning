...
public:
  ...
  // Vars needed for computing covariance
  int n_back = 20;
  int curr_saved = 0;
  // last measured stored in order: pos_x, pos_y, twist_x, twist_z
  float last_measured[4][20];
  ...
  CLaserOdometry2D()
{
  ...
  pn.param<std::string>("laser_frame_id", laser_frame_id, "/scout_1/hokuyo_link");
...
bool CLaserOdometry2DNode::setLaserPoseFromTf()
{
  ...
  try
  {
    while (ros::Time::now() == ros::Time(0)) {
      ROS_WARN_STREAM("No messages received on /clock yet. Is master running?");
      ros::Duration(1.0).sleep();
    }
    tf_listener.waitForTransform(base_frame_id, laser_frame_id, ros::Time::now(), ros::Duration(5.0));
    ROS_INFO_STREAM("Laser frame ID: " << laser_frame_id);
    tf_listener.lookupTransform(base_frame_id, laser_frame_id, ros::Time::now(), transform);
    retrieved = true;
  }
...
void CLaserOdometry2DNode::publish()
{
  ...
  // Compute covariance values and update
  if (curr_saved == n_back) {
    // Roll old values off buffer
    for (int j = 0; j < 4; j++) {
      for (int i = 1; i < n_back; i++) {
        last_measured[j][i-1] = last_measured[j][i];
      }
    }
  } else {
    curr_saved += 1;
  }
  last_measured[0][curr_saved-1] = robot_pose_.translation()(0);
  last_measured[1][curr_saved-1] = robot_pose_.translation()(1);
  last_measured[2][curr_saved-1] = lin_speed;
  last_measured[3][curr_saved-1] = ang_speed;
  float cov_values[4] = {0.1, 0.1, 0.5, 0.1};
  if (curr_saved == n_back) {
    float means[4] = {0, 0, 0, 0};
    for (int j = 0; j < 4; j++) {
      for (int i = 0; i < n_back; i++) {
        means[j] += last_measured[j][i]/n_back;
      }
    }
    for (int j = 0; j < 4; j++) {
      for (int i = 0; i < n_back; i++) {
        float square_dif = (last_measured[j][i] - means[j]) *
          (last_measured[j][i] - means[j]);
        cov_values[j] += square_dif / (n_back - 1);
      }
    }
  }
  // Set covariance matrices
  odom.pose.covariance[0] = cov_values[0];
  odom.pose.covariance[6*1+1-1] = cov_values[1];
  odom.twist.covariance[0] = cov_values[2];
  odom.twist.covariance[6*5+6-1] = cov_values[3];
  //publish the message
